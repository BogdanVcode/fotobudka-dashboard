...
root=pathlib.Path(__file__).resolve().parent.parent;out=root/'drawings/revision-09';d=json.loads((out/'design.json').read_text(encoding='utf8'));parts=d['parts']
...
W,H=2383.94,1683.78;pdf=canvas.Canvas(str(root/'output/pdf/Фотобудка-09-креслення.pdf'),pagesize=(W,H));pdf.setTitle('Фотобудка Славутич — зведені креслення 09');svg=[];sheets=[]
...
def page(title,note):
 global svg;svg=[];rect(20,20,W-40,H-40);text(45,55,title,25,True);text(45,82,note,13);line(20,H-68,W-20,H-68);text(40,H-40,'ФОТОБУДКА СЛАВУТИЧ · 09 · 17.09.2026 · мм · А1 · не масштабувати з екрана',13);text(W-650,H-40,'Автор: Богдан · комплект для узгодження з майстром',13)
def save(name):
...
def panel(p,x,y,cw,ch,details=True):
 ...
 text(x+10,y+ch-10,'Об’ємний вузол з кишенями; 162 мм — глибина.' if p.get('assembly') else 'Товщина / габарит торця: '+fmt(thin)+' мм',10)
...
for axes,x,y,bw,bh,title in [((0,2),80,180,950,1170,'Вид збоку: лавка → модуль'),((1,2),1280,180,950,1170,'Вид на монітор: праворуч — сканер')]:
...
for i,p in enumerate(special):
 ...
 for j,(u,v,w,h) in enumerate(loops[1:]):text(x+15,1210+j*24,f'{j+1}. X {fmt(u)} / Y {fmt(v)}; {fmt(w)} × {fmt(h)} мм',12)
...
save('06-assembly.svg');pdf.save()
(out/'index.html').write_text('<!doctype html><html lang="uk"><meta charset="utf-8"><title>Зведені креслення 09</title><style>body{font:18px Arial;background:#eef1f5;padding:25px}img{width:100%;background:white;margin:20px 0}a{color:#235474}@media print{img{break-after:page}header{display:none}}</style><header><h1>Зведені креслення · 09</h1><p>Усі ДСП — аркуш 01. Увесь каркас — аркуш 02. Далі компонування, вирізи, світлові панелі й монтаж.</p><a href="../../output/pdf/Фотобудка-09-креслення.pdf">Завантажити PDF А1</a></header>'+''.join(f'<a href="{s}"><img src="{s}" alt="{s}"></a>' for s in sheets),encoding='utf8')
print('DRAWINGS09:',len(panels),'DSP objects;',len(frames),'frame parts;',len(sheets),'A1 sheets;',total,'m profile')
