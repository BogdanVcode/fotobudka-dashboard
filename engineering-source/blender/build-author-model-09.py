...
out=root/'drawings/revision-09';out.mkdir(parents=True,exist_ok=True)
...
added=bpy.data.collections.new('09 — додані вузли; компонування на перевірку');bpy.context.scene.collection.children.link(added)
materials={}
for name,color in {'white':(1,1,1,1),'black':(.05,.06,.075,1),'edge':(.6,.65,.69,1),'light':(1,1,1,1),'cushion':(.95,.95,.95,1),'glass':(.7,.85,.9,.2),'screen':(.1,.2,.25,1),'mirror':(.7,.8,.85,1),'floor':(.5,.55,.6,1)}.items():
    m=bpy.data.materials.new('09 '+name);m.diffuse_color=color;materials[name]=m
records=[]
...
worldbox('Brother DS-740D · справа від монітора',[175,-465,650],[63,45,301])
worldbox('Опора сканера · знімний кронштейн',[175,-470,642],[100,60,8],material='edge')
...
routes=[
 ...
 ('USB · сканер → концентратор',[(235,-440,660),(385,-480,670),(770,-480,250),(700,-135,40)]),
...
data={'version':9,'date':'2026-09-17','source':'БУДКА.blend','sourceSHA256':hashlib.sha256(source.read_bytes()).hexdigest(),'offsetMm':list(off*1000),'parts':records,'productionReleased':False,'notes':['Авторська геометрія 17.09.2026 збережена без змін.','Принтери стоять на авторській полиці. Спалах ліворуч, якщо дивитися на монітор.','EcoFlow і ПК внизу. Сканер справа від монітора.','Дроти — маршрути, не електрична принципова схема.','Передня деталь з кишенями є об’ємним вузлом, не плитою 162 мм.']}
(out/'design.json').write_text(json.dumps(data,ensure_ascii=False),encoding='utf-8')
(root/'dashboard/design09.js').write_text('window.AUTHOR_DESIGN='+json.dumps(data,ensure_ascii=False)+';',encoding='utf-8')
bpy.ops.wm.save_as_mainfile(filepath=str(out/'БУДКА-доповнена-09.blend'))
print('REV09:',len(records),'parts;',sum(p['authorGeometry'] for p in records),'author objects')
