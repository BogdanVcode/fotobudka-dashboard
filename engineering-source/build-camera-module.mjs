...
if(!model.includes('const cableMaterial='))model=model.replace('let cable;','let cable;const cableMaterial=new T.LineBasicMaterial({color:0x228479});').replace('new T.LineBasicMaterial({color:0x228479}));scene.add(cable);','cableMaterial);scene.add(cable);');
...
if(!index.includes('id="up"'))index=index.replace('<button id="center">','<button id="up">Вгору 25 мм</button><button id="down">Вниз 25 мм</button><button id="center">');
